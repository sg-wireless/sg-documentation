...
- **Application Mode**: The normal application mode and this mode is enabled by
  default for all end- devices that don't have LCT mode enabled.
...
- **Certification Mode Once Operation Type**: The end-device will be set to
  ...
  memory in the flash is erased.
     ...
     port 224, the device will switch automatically to the normal mode and will
     no longer operate in the certification mode.
...
- ``CONFIG_LORA_LCT_MODE`` "LoRa Certification test mode"
  ...
  disabled, the device will operate in ``Only Application mode Operation type``
...
- ``CONFIG_LORA_LCT_OPERATE_AFTER_RESET``
  ...
  - Reset caused by TCL command DutResetReq:
    ...
    After initialising the LoRa stack, it will do automatically a join-req and
    the system will continue working with the TCL.
