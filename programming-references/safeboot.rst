...
-  ``software-reset``: the system can be triggered to reset in
   ``safeboot-mode`` by a software call to this function
   ``bootif_safeboot_soft_reset()``. In ``micropython`` variant, there is a
   shortcut key ``Ctrl + F`` which triggers the same function call.
...
-  ``Hardware-Safeboot-Button``: the system also, offers another way using an
   ...
   utilize this feature, then it could be activated from the SDK
   ``menu-config`` options and can be configggered to use another GPIO pin than
   the default one.

   In this triggerring mode, the system offers as well, some extra options
   depending on the switch press holding time. There are three different
   holding times:
   ...
   -  ``Safeboot in previous OTA Firmware Hold Time`` (default 3 <= T < 7 sec):
      ...
      from the latest.
   -  ``Safeboot in Factory Firmware Hold Time`` (default 7 <= T sec): The
      system boots from the factory firmware in safeboot mode.
...
.. image:: images/safeboot.png
   :alt: Firmware/Bootloader Safeboot Interaction
...
.. image:: images/booting-screen-shot.png
   :alt: Booting Screen Shot
...
.. image:: images/main-menu-config.png
   :alt: menu-config main screen

Then select ``*** SDK Platform`` and in the next screen, select
``F1 Platform``, then ``Safeboot Feature``, then you get this screen:

.. image:: images/safeboot-config.png
   :alt: menu-config safeboot screen
