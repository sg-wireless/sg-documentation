...
Ctrl. Personal account settings are accessible for everyone, while the
organization settings are only open to users who have ``Store Owner`` or
``Admin`` role.
...
2. Under the ``Account`` tab, expand the ``Profile`` section. You can update your
...
2. Under the ``Account`` tab, expand the ``Credentials`` section. You can update
